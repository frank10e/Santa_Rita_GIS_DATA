...
```{r}
# Convert SRER_Pastures to an sf object
SRER_Pastures_sf <- st_as_sf(SRER_Pastures, coords = c("UTM Center X", "UTM Center Y"), crs = 32612)

# Convert santa_rita_data to an sf object
santa_rita_data_sf <- st_as_sf(santa_rita_data, coords = c("UTM Center X", "UTM Center Y"), crs = st_crs(SRER_Pastures_sf))

# Filter specific transect names from santa_rita_data
specific_transects <- santa_rita_data_sf[santa_rita_data_sf$`Transect Name` %in% c("12A_01", "12A_02", "12A_03", "12A_04", "12A_05", "12A_06"), ]

# Update Value column with your provided values
specific_transects$Value <- c(26.1, 42, 43.9, 53.7, 36.4, 42)

# Convert to SpatialPointsDataFrame
specific_transects_sp <- as(specific_transects, "Spatial")
grd_sp <- as(st_make_grid(specific_transects, cellsize = 10, what = "centers"), "Spatial")

# IDW interpolation
idw_result <- gstat::idw(Value ~ 1, locations = specific_transects_sp, newdata = grd_sp)

# Convert IDW results to raster
r <- raster(extent(SRER_Pastures_sf[SRER_Pastures_sf$Pasture == "12A", ]))
res(r) <- 10 # or choose an appropriate resolution
idw_raster <- rasterize(as(idw_result, "SpatialPointsDataFrame"), r, field = "var1.pred")
# Convert SRER_Pastures_sf to a 2D object
SRER_Pastures_sf_2D <- st_zm(SRER_Pastures_sf)

#  rasterize using the 2D object
pasture_mask <- rasterize(SRER_Pastures_sf_2D[SRER_Pastures_sf_2D$Pasture == "12A", ], r, field = 1)
clipped_raster <- mask(idw_raster, pasture_mask)
```
...
tm_shape(clipped_raster) + 
    tm_raster(palette = "RdBu", title = "Interpolated Value") +
    tm_shape(SRER_Pastures_sf[SRER_Pastures_sf$Pasture == "12A", ]) + 
    tm_borders(lwd = 1.5, col = 'black') +
    tm_shape(specific_transects) +
    tm_dots(col = "Value", size = 0.5, palette = "RdBu", title = "Value") +
    tm_text('Transect Name', size = 0.7) +
    tm_layout(main.title = "SRER Pasture 12A with Specific Transect Names")
...
# Update Value column with provided values
specific_transects$Value <- c(26.1, 42, 43.9, 53.7, 36.4, 42)
...
e <- st_bbox(SRER_Pastures_sf[SRER_Pastures_sf$Pasture == "12A", ])
r_cropped <- crop(idw_raster, e)
# Remove Z and M dimensions
SRER_Pastures_sf_2D <- st_zm(SRER_Pastures_sf)

# Rasterize again using the 2D object
pasture_mask <- rasterize(SRER_Pastures_sf_2D[SRER_Pastures_sf_2D$Pasture == "12A", ], r, field = 1)
clipped_raster <- mask(r_cropped, pasture_mask)

# Extend the raster to desired extent
desired_extent <- extent(SRER_Pastures_sf)
extended_raster <- extend(clipped_raster, desired_extent)
...
idw_result <- idw(Value ~ 1, locations = specific_transects_sp, newdata = grd_sp)
...
tm_shape(r) + 
  tm_raster(n=10, palette = "RdBu", auto.palette.mapping = TRUE, title="Interpolated Value") + 
  tm_shape(specific_transects) + tm_dots(size=0.2) +
...
idw_result <- idw(Value ~ 1, locations = specific_transects_sp, newdata = grd)
r <- raster(idw_result)
r_clipped <- crop(r, extent(santa_rita_data_plot))

SRER_Pastures_sf <- st_as_sf(SRER_Pastures)

SRER_Pastures_polygons <- st_buffer(SRER_Pastures_sf, dist = 10) 

specific_transects_polygons <- st_buffer(specific_transects, dist = 10) 


tm_shape(r_clipped) + 
  tm_raster(style="cont", n=10, palette="RdBu", title="Interpolated Value") +
...
grd_poly <- st_make_grid(santa_rita_data_plot, cellsize = 50000)
grd <- st_sample(st_zm(grd_poly), size = 50000, type = "regular")
grd_sf <- st_as_sf(grd, crs = st_crs(santa_rita_data_plot))  # Note the change here.
...
santa_rita_12A_sf <- santa_rita_data_plot[santa_rita_data_plot$Pasture == "12A",]
raster_12A <- raster(extent(santa_rita_12A_sf))

r_resampled <- resample(r, raster_12A, method = "bilinear")


raster_12A_mask <- mask(raster_12A, raster_12A)
r_masked <- mask(r_resampled, raster_12A_mask)

crs_value <- st_crs(santa_rita_data_plot)$proj4string
crs(r_masked) <- crs_value


tm_shape(r_masked) + 
  tm_raster(style="cont", n=10, palette="RdBu", title="Interpolated Value") +
  tm_shape(santa_rita_data_plot[santa_rita_data_plot$Pasture == "12A",]) +  
...
# 使用上面的函数来更新specific_transects的定义和其Value
